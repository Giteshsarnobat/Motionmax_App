...
const API_URL = "https://motionmax-backend-production.up.railway.app";
...
function removeBanner() {
  const old = document.getElementById("loginBanner");
  if (old) old.remove();
}
...
function redirectByRole(role) {
  window.location.href =
    role === "admin" ? "AdminDashboard.html" : "Dashboard.html";
}
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        showBanner("Invalid email or password. Please try again.");
        showFieldError(passInput, "Incorrect password.");
      } else if (response.status === 403) {
        showBanner("Your account has been deactivated. Contact support.");
